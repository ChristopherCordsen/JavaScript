const Calculator = {
    ...
    Display_Value: '0',
    //this will hold the first operand for any expressions, we set it to nill for now
    First_Operand: null,
...
function Input_Digit(digit) {
    ...
    // Display_Value to the key that was clikcked.
    if (Wait_Second_Operand === true) {
        Calculator.Display_Value = digit;
        Calculator.Wait_Second_Operand = false;
    } else {
        //this overwrites Display_Value if the current calue is )
...
function Handle_Operator(Next_Operator) {
    ...
    //displayed on the screen to a number andthen store the result in 
    ...
    if (First_Operand == null) {
      ...
      //if operator already exists, property lookup is perfirned for the operator 
      //in th Perform_Calculation object and the function that matches the 
...
Update_Display();
// this section monitors butto clicks 
const keys = document.querySelector('.calculator-keys');
keys.addEventListener('click', (event) => {
    //the target variable is an object that represents the element 
    //that was clicked 
    const { target } = event;
    //if the elemenet was clicked on is not a button, exit the function 
